<template>
    ...
    <div v-if="mode === 'edit'">
      ...
      <div class="mt-auto d-grid gap-2 d-md-flex justify-content-md-between">
        ...
        <button @click="saveTest" class="btn btn-success mb-4">Сохранить изменения</button>
      </div>
    </div>
...
onMounted(async () => {
  try {
    let url = `${endpoints.expert_system.getSkillsForRedact}/${props.id}/`
    const skillsResponse = await apiClient.get(url)
    skills.value = skillsResponse.data
    const testResponse = await apiClient.get(endpoints.expert_system.getTestForRedact, { id: props.id })
    test.value = testResponse.data
  } catch (err) {
    console.error('Ошибка загрузки данных:', err)
    errors.value.push('Не удалось загрузить данные теста')
  }
})
</script>
