<template>
  <div class="priority-management">
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h5 class="mb-0">
        <i class="fas fa-exclamation-triangle me-2"></i>Управление приоритетами задач
...
export default {
  ...
  methods: {
    ...
    async deletePriority(priority) {
      ...
      try {
        await projectManagementApi.deleteTaskPriority(priority.id)
        this.closeConfirmDialog()
    ...
    async savePriority() {
      try {
        ...
        if (this.isEditMode) {
          await projectManagementApi.updateTaskPriority(this.currentPriority.id, this.formData)
          this.showSuccess('Приоритет обновлен')
        } else {
          await projectManagementApi.createTaskPriority(this.formData)
